...
const createLiquidEffect = (texture: THREE.Texture, opts: { strength?: number; freq?: number }) => {
  ...
  return effect as Effect & { uniforms: Map<string, THREE.Uniform> };
}
...
const PixelBlast = ({
  ...
  const threeRef = useRef<ThreeSetup | null>(null)
  const prevConfigRef = useRef<{ [key: string]: boolean | number } | null>(null)
  useEffect(() => {
    const container = containerRef.current
    if (!container) return
    speedRef.current = speed
    const needsReinitKeys: Array<keyof typeof cfg> = ["antialias", "liquid", "noiseAmount"]
    const cfg: { [key: string]: boolean | number } = { antialias, liquid, noiseAmount }
    ...
    if (mustReinit) {
      ...
      let liquidEffect: (Effect & { uniforms: Map<string, THREE.IUniform> }) | undefined
      if (liquid) {
      ...
      const mapToPixels = (e: PointerEvent) => {
        ...
        const fy = ((container as HTMLDivElement).clientHeight - (e.clientY - rect.top)) * scaleY
        return {
          fx,
          fy,
      ...
      const animate = () => {
        ...
        if (composer) {
          ...
          composer.passes.forEach((p: any) => { // p is a Pass, but its type is not directly exported from postprocessing
            const effs = p.effects
            if (effs)
              effs.forEach((eff: Effect) => {
                const u = (eff as Effect & { uniforms: Map<string, THREE.IUniform> }).uniforms?.get("uTime")
