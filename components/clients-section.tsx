...
export function ClientsSection() {
  ...
  const clients: Client[] = [
    ...
    { name: "Tower Bersama", logo: "/tower-bersama-logo.jpg", description: "Tower Bersama Infrastructure is a leading independent telecommunication tower provider in Indonesia." },
    { name: "Peruri", logo: "/peruri-logo.jpg", description: "Peruri is the state-owned company responsible for printing the Indonesian rupiah currency and other security documents." },
    { name: "INKA", logo: "/inka-logo.jpg", description: "INKA (Industri Kereta Api) is a state-owned rolling stock manufacturer in Indonesia." },
    { name: "Astra Infra", logo: "/astra-infra-logo.jpg", description: "Astra Infra is a leading investor and operator in the Indonesian infrastructure sector, focusing on toll roads and ports." },
    { name: "Nestle", logo: "/generic-bird-nest-logo.png", description: "Nestlé is a Swiss multinational food and drink processing conglomerate corporation headquartered in Vevey, Vaud, Switzerland." },
    { name: "Infomedia", logo: "/infomedia-logo.jpg", description: "Infomedia Nusantara is a subsidiary of Telkom Indonesia, focusing on business process management and contact center services." },
    { name: "Unpar", logo: "/unpar-university-logo.jpg", description: "Parahyangan Catholic University (Unpar) is one of the oldest and most prestigious private universities in Indonesia." },
    { name: "Cibavision", logo: "/cibavision-logo.jpg", description: "CIBA Vision was a manufacturer of contact lenses and lens care products, now part of Alcon." },
  ]
    ...
    <>
      <section className="py-32 relative overflow-hidden">
        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          ...
          {/* Client logos grid */}
          <motion.div
            className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6 gap-6 mb-15"
          ...
          >
            {clients.map((client, index) => (
              <motion.div key={index} variants={itemVariants} onClick={() => setSelectedClient(client)}>
                <Card className="bg-white/50 dark:bg-card/30 border-primary/10 hover:border-primary/30 hover:bg-white dark:hover:bg-card/60 transition-all duration-300 hover:shadow-lg hover:-translate-y-1 backdrop-blur-sm group cursor-pointer">
